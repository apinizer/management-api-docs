...
Match requests to specific API paths.
...
### Scenario 2: Multi-Host Support

Route requests based on host name (useful for multi-tenant scenarios).
...
### Scenario 3: Header-Based Versioning

Route requests based on API version header.
...
Disable buffering for read-only endpoints to improve performance.
...
- [Update API Proxy](../crud/update-api-proxy.md) - Update API proxy configuration
