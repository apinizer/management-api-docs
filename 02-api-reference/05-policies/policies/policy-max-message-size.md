...
### Scenario 1: Limit Request Size to 1MB

Limit request body size to 1MB to prevent large uploads.
...
```bash
curl -X POST \
  "https://demo.apinizer.com/apiops/projects/MyProject/apiProxies/MyAPI/policies/limit-1mb/" \
  -H "Authorization: Bearer YOUR_TOKEN" \
  -H "Content-Type: application/json" \
  -d @request-body.json
```

### Scenario 2: Limit Response Size

Limit response body size to prevent large responses.
...
```json
{
  ...
  "policy": {
    "type": "policy-max-message-size",
    "description": "Limit response size to 10MB",
    "active": true,
...
### Scenario 3: Endpoint-Specific Size Limit

Limit size for a specific endpoint only.
...
```json
{
  ...
  "policy": {
    "type": "policy-max-message-size",
    "description": "Limit upload endpoint to 5MB",
    "active": true,
    "maxMessageSize": 5242880
  }
}
