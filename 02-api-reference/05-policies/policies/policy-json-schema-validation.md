...
### Scenario 1: Validate Simple Object

Validate a simple JSON object with required fields.
...
```bash
curl -X POST \
  "https://demo.apinizer.com/apiops/projects/MyProject/apiProxies/MyAPI/policies/validate-user/" \
  -H "Authorization: Bearer YOUR_TOKEN" \
  -H "Content-Type: application/json" \
  -d @request-body.json
```
...
### Scenario 2: Validate Nested Object

Validate a nested JSON object using JSONPath.
...
```json
{
  ...
  "policy": {
    "type": "policy-json-schema-validation",
    "description": "Validate nested data object",
    "active": true,
    "pathForBody": "$.data",
    "schemaDefinitionList": [
      {
...
```json
{
  ...
  "policy": {
    "type": "policy-json-schema-validation",
    "description": "Validate array of items",
    "active": true,
    "pathForBody": "$.items",
    "schemaDefinitionList": [
      {
