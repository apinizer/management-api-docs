...
### Scenario 1: Add Custom Headers (Groovy)

Add custom headers to request using Groovy script.
...
```bash
curl -X POST \
  "https://demo.apinizer.com/apiops/projects/MyProject/apiProxies/MyAPI/policies/add-headers/" \
  -H "Authorization: Bearer YOUR_TOKEN" \
  -H "Content-Type: application/json" \
  -d @request-body.json
```

### Scenario 2: Modify Response Body (JavaScript)

Modify response body using JavaScript script.
...
```json
{
  ...
  "policy": {
    "type": "policy-script",
    "description": "Modify response body",
    "active": true,
    "executionType": "SYNC",
    "scriptLanguage": "JAVASCRIPT",
    "scriptBody": "var messageContext = messageContext;\nvar response = messageContext.getResponse();\nvar body = response.getBodyAsString();\nvar jsonBody = JSON.parse(body);\njsonBody.timestamp = new Date().toISOString();\nresponse.setBody(JSON.stringify(jsonBody));"
  }
}
```

### Scenario 3: Asynchronous Logging

Log request information asynchronously without blocking the request.
