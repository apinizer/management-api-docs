...
### Scenario 1: Basic Throttling by API Key

Throttle requests based on API key header, allowing 100 requests per minute per API key.
...
```json
{
  ...
  "policy": {
    "type": "policy-api-based-throttling",
    "description": "Throttle by API key - 100 req/min",
    "active": true,
    "targetVariableForIdentity": {
      "type": "HEADER",
      "headerName": "X-API-Key"
...
```bash
curl -X POST \
  "https://demo.apinizer.com/apiops/projects/MyProject/apiProxies/MyAPI/policies/api-key-throttle/" \
  -H "Authorization: Bearer YOUR_TOKEN" \
  -H "Content-Type: application/json" \
  -d @request-body.json
```

### Scenario 2: Throttling by Client IP

Throttle requests based on client IP address, allowing 50 requests per minute per IP.
...
```json
{
  ...
  "policy": {
    "type": "policy-api-based-throttling",
    "description": "Throttle by IP - 50 req/min",
    "active": true,
    "targetVariableForIdentity": {
      "type": "CONTEXT",
...
### Scenario 3: VIP Users with Higher Limits

Allow VIP users (identified by header value) to have higher rate limits.
...
```json
{
  ...
  "policy": {
    "type": "policy-api-based-throttling",
    "description": "Throttling with VIP support",
    "active": true,
    "targetVariableForIdentity": {
      "type": "HEADER",
