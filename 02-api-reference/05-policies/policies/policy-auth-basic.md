...
### Scenario 1: Standard Basic Authentication

Validate Basic Authentication from Authorization header and add user to custom header.
...
```json
{
  ...
  "policy": {
    "type": "policy-auth-basic",
    "description": "Basic authentication from Authorization header",
    "active": true,
    "usernameVar": {
      "type": "HEADER",
      "headerName": "Authorization"
...
```bash
curl -X POST \
  "https://demo.apinizer.com/apiops/projects/MyProject/apiProxies/MyAPI/policies/basic-auth/" \
  -H "Authorization: Bearer YOUR_TOKEN" \
  -H "Content-Type: application/json" \
  -d @request-body.json
```
...
### Scenario 2: Clear Authorization Header After Validation
...
```json
{
  ...
  "policy": {
    "type": "policy-auth-basic",
    "description": "Basic auth - clear header after validation",
    "active": true,
    "usernameVar": {
      "type": "HEADER",
      "headerName": "Authorization"
...
### Scenario 3: Extract from Custom Header

Extract Basic Authentication from a custom header instead of Authorization header.
...
```json
{
  ...
  "policy": {
    "type": "policy-auth-basic",
    "description": "Basic auth from custom header",
    "active": true,
    "usernameVar": {
      "type": "HEADER",
