...
### Scenario 1: Add Header from Context

Add a timestamp header using context value.
...
### Scenario 2: Modify Value with Masking

Mask credit card number in request body.
...
```json
{
  ...
  "policy": {
    "type": "policy-business-rule",
    "description": "Remove internal header",
    "active": true,
    "actionList": [
      {
        "actionType": "DELETE",
...
### Scenario 4: Stop Flow Conditionally

Stop request flow if condition is met.
...
```json
{
  ...
  "policy": {
    "type": "policy-business-rule",
    "description": "Stop unauthorized requests",
    "active": true,
    "actionList": [
      {
        "actionType": "STOP"
