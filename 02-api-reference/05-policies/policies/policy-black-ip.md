...
### Scenario 1: Block Specific IP Addresses

Block requests from specific malicious IP addresses.
...
```json
{
  ...
  "policy": {
    "type": "policy-black-ip",
    "description": "Block specific malicious IPs",
    "active": true,
    "ipList": [
      "192.168.1.100",
      "192.168.1.101",
...
```bash
curl -X POST \
  "https://demo.apinizer.com/apiops/projects/MyProject/apiProxies/MyAPI/policies/block-ips/" \
  -H "Authorization: Bearer YOUR_TOKEN" \
  -H "Content-Type: application/json" \
  -d @request-body.json
```

### Scenario 2: Block IP Ranges (CIDR)
...
### Scenario 3: Block IPv6 Addresses

Block IPv6 addresses and ranges.
...
```json
{
  ...
  "policy": {
    "type": "policy-black-ip",
    "description": "Block IPs from custom header",
    "active": true,
    "targetVariableForIP": {
      "type": "HEADER",
