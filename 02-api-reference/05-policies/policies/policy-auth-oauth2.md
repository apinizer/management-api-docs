...
### Scenario 1: OAuth2 with Expiration and Refresh Tokens

Configure OAuth2 authentication with token expiration and refresh token support.
...
```json
{
  ...
  "policy": {
    "type": "policy-auth-oauth2",
    "description": "OAuth2 with 1 hour expiration and refresh tokens",
    "active": true,
    "tokenNeverExpires": false,
    "tokenExpiresInAmount": 3600,
    "tokenExpiresInUnit": "SECONDS",
...
```bash
curl -X POST \
  "https://demo.apinizer.com/apiops/projects/MyProject/apiProxies/MyAPI/policies/oauth2-auth/" \
  -H "Authorization: Bearer YOUR_TOKEN" \
  -H "Content-Type: application/json" \
  -d @request-body.json
```

### Scenario 2: OAuth2 Client Credentials Grant

Configure OAuth2 authentication using client credentials grant type.
...
### Scenario 3: OAuth2 with Never-Expiring Tokens

Configure OAuth2 authentication with tokens that never expire.
...
```json
{
  ...
  "policy": {
    "type": "policy-auth-oauth2",
    "description": "OAuth2 with never-expiring tokens",
    "active": true,
