...
### Scenario 1: Daily Quota by API Key

Set a daily quota of 10,000 requests per API key.
...
```json
{
  ...
  "policy": {
    "type": "policy-api-based-quota",
    "description": "Daily quota - 10000 requests per day",
    "active": true,
    "targetVariableForIdentity": {
      "type": "HEADER",
      "headerName": "X-API-Key"
...
```bash
curl -X POST \
  "https://demo.apinizer.com/apiops/projects/MyProject/apiProxies/MyAPI/policies/daily-quota/" \
  -H "Authorization: Bearer YOUR_TOKEN" \
  -H "Content-Type: application/json" \
  -d @request-body.json
```

### Scenario 2: Monthly Quota by Client IP

Set a monthly quota of 1,000,000 requests per client IP.
...
```json
{
  ...
  "policy": {
    "type": "policy-api-based-quota",
    "description": "Monthly quota - 1M requests per IP",
    "active": true,
    "targetVariableForIdentity": {
      "type": "CONTEXT",
      "contextValue": "CLIENT_IP"
    },
    "messageCountForInterval": 1000000,
...
### Scenario 3: VIP Users with Higher Quota

Allow VIP users (identified by header value) to have higher quota limits.
...
```json
{
  ...
  "policy": {
    "type": "policy-api-based-quota",
    "description": "Quota with VIP support",
    "active": true,
    "targetVariableForIdentity": {
      "type": "HEADER",
