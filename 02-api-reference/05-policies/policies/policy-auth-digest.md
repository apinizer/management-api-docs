...
### Scenario 1: Standard Digest Authentication

Validate Digest Authentication from Authorization header with all required fields.
...
```json
{
  ...
  "policy": {
    "type": "policy-auth-digest",
    "description": "Digest authentication from Authorization header",
    "active": true,
    "usernameVar": {
      "type": "HEADER",
      "headerName": "Authorization"
...
```bash
curl -X POST \
  "https://demo.apinizer.com/apiops/projects/MyProject/apiProxies/MyAPI/policies/digest-auth/" \
  -H "Authorization: Bearer YOUR_TOKEN" \
  -H "Content-Type: application/json" \
  -d @request-body.json
```

### Scenario 2: Minimal Digest Authentication

Validate Digest Authentication with only username and password (without nonce and created).
...
```json
{
  ...
  "policy": {
    "type": "policy-auth-digest",
    "description": "Digest auth - minimal configuration",
    "active": true,
    "usernameVar": {
      "type": "HEADER",
      "headerName": "Authorization"
...
### Scenario 3: Clear Authorization Header After Validation

Validate credentials and remove Authorization header before forwarding to backend.
...
```json
{
  ...
  "policy": {
    "type": "policy-auth-digest",
    "description": "Digest auth - clear header after validation",
    "active": true,
    "usernameVar": {
      "type": "HEADER",
      "headerName": "Authorization"
