...
### Scenario 1: Add Policy Without Deployment

Add a policy but don't deploy immediately.
...
### Scenario 2: Add Policy at Specific Order

Add a policy at a specific position in the pipeline.
...
```json
{
  ...
  "policy": {
    "type": "policy-authentication-basic",
...
**Note:** If `order` is null or >= current policy list size, the policy is added at the end.

### Scenario 3: Add Policy with Complex Condition

Add a policy with nested condition rules.
...
```json
{
  ...
  "policy": {
    "type": "policy-black-ip",
    "description": "Block IPs with complex condition",
    "active": true,
    "ipList": ["192.168.1.100"],
    "policyCondition": {
      "conditionRuleList": [
