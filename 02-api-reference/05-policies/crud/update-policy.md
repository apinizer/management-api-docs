...
### Scenario 1: Update Policy Configuration Only

Update policy fields without changing order or scope.
...
```json
{
  ...
  "policy": {
    ...
    "description": "Updated IP blacklist",
    "active": true,
    "ipList": ["192.168.1.100", "10.0.0.50", "172.16.0.1"]
  }
}
```

**Note:** Setting `order: null` keeps the existing order.

### Scenario 2: Update and Deploy

Update policy configuration and deploy immediately.
...
```json
{
  ...
  "policy": {
    "type": "policy-api-based-throttling",
    "description": "Updated throttling",
    "active": true,
    "messageCountForInterval": 300,
    "throttlingInterval": "ONE_MINUTE",
    "intervalPeriodLength": 1,
    "intervalWindowType": "FIXED"
  }
...
```json
{
  ...
  "policy": {
    "type": "policy-black-ip",
    "description": "Disabled IP blocking",
    "active": false,
