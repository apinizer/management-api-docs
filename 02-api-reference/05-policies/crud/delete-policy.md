...
### Scenario 1: Delete Policy from All Endpoints

Delete a policy that applies to all endpoints.
...
### Scenario 2: Delete Policy from Specific Endpoint

Delete a policy that applies only to a specific endpoint.
...
### Scenario 3: Delete Multiple Policies

To delete multiple policies, make separate DELETE requests for each policy.

```bash
# Delete policy 1
curl -X DELETE \
  "https://demo.apinizer.com/apiops/projects/MyProject/apiProxies/MyAPI/policies/policy1/" \
  -H "Authorization: Bearer YOUR_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"operationMetadata": {"targetScope": "ALL", "targetPipeline": "REQUEST", "deploy": false}}'

# Delete policy 2
curl -X DELETE \
  "https://demo.apinizer.com/apiops/projects/MyProject/apiProxies/MyAPI/policies/policy2/" \
  -H "Authorization: Bearer YOUR_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"operationMetadata": {"targetScope": "ALL", "targetPipeline": "REQUEST", "deploy": false}}'

# Deploy after all deletions
curl -X POST \
  "https://demo.apinizer.com/apiops/projects/MyProject/apiProxies/MyAPI/deploy/" \
  -H "Authorization: Bearer YOUR_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"environmentNameList": ["production"]}'
```
